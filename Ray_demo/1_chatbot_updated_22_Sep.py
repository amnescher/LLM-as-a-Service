    ...
    if st.session_state.show_logged_in_message:
        ...
        if logedin_username == "admin":
            ...
            if selected_button_label:
                ...
                st.session_state.newchat = "newchat"
                st.session_state.messages = [
            ...
            if search_choice == "Document Search":
                ...
                if st.sidebar.button("Confirm"):
                    if selected_collection is not None:
                        ...
                        st.sidebar.success(
                            f"Collection {selected_collection} selected!"
                        )
            ...
            if st.session_state.messages[-1]["role"] != "assistant":
                with st.chat_message("assistant"):
                    with st.spinner("Thinking..."):
                        ...
                        search_choice = "ai_assistance"
                        URL = f"http://localhost:{PORT_NUMBER}/predict/?text={prompt}&mode={search_choice}&username={username}&newchat={newchat}&conversation_number={conversation_number}&messages={st.session_state.messages}"
