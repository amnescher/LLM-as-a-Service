...
from app.models import LoginUser, Data, Input
...
async def retrieve_latest_conversation(
    input: Input,
    current_user: LoginUser = Security(get_current_active_user),
    db: Session = Depends(get_db),
):
...
async def retrieve_all_conversations(
    ...
    Returns:
        - A dictionary containing the number of conversations retrieved.
    ...
    try:
        ...
        return {"number_of_conversations": len(conversations)}
