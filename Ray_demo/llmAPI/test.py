...
@VDB_app.post("/")
async def VectorDataBase( request: VDBaseInput = Depends(), file: Optional[UploadFile] = File(None)):
        mode = request.mode
        ...
        if VDB_type == "Weaviate":
                ...
                collection_name = Database().add_collection({"username": request.username, "collection_name": request.collection_name})
