...
def clear_chat_history():
    requests.post(url=f"http://127.0.0.1:{port}/clearMem")
    st.session_state.messages = [{"role": "assistant", "content": "How may I assist you today?"}]

def clear_videos():
    requests.post(url=f"http://127.0.0.1:{port}/clearvideos")
    st.sidebar.success = "Cleared database."
def clear_document():
    requests.post(url=f"http://127.0.0.1:{port}/clearDocs")
    st.sidebar.success = "Cleared database."
...
def upload_videos(user_input, port):
    try:
        ...
        data = {"url": user_input}
        url = f"http://127.0.0.1:{port}/video_loading"
...
if st.sidebar.button("Upload document"):
        if uploaded_pdf is not None:
                ...
                url = f"http://127.0.0.1:{port}/document_loading"
...
if st.session_state.messages[-1]["role"] != "assistant":
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                response = requests.post(url=f"http://127.0.0.1:{port}/predict", json={'prompt': prompt,'mode':search_choice, 'messages': st.session_state.messages})#data=json.dumps(prompt))#generate_llama2_response(llm,prompt)
                print('resp', response)
