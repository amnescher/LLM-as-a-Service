...
def get_llm(llm_name, model_temperature, max_tokens=256):
    ...
    if llm_name == "Falcon-7B":
    ...
    elif llm_name == "Falcon-40B":
        model_name = '/mnt/tmp/falcon'
        ...
        llm = HuggingFacePipeline(pipeline = F40B_Pipeline)
...
def search_pdf(query,llm_a,embed):
    ...
    elif(llm_a == 'Falcon-40B' and embed =='SBERT'):    
        llm_d,embed_model = get_llm('Falcon-40B', 0.1, max_tokens=1024)
    #embed_model = LangchainEmbedding(OpenAIEmbeddings())
        embed_model = LangchainEmbedding(HuggingFaceEmbeddings(model_name='sentence-transformers/all-mpnet-base-v2')) #LangchainEmbedding(OpenAIEmbeddings())#
        service_context = ServiceContext.from_defaults(llm_predictor=LLMPredictor(llm=llm_d),embed_model=embed_model,
                                                chunk_size=1024)
    #storage_context = StorageContext.from_defaults(vector_store=DeepLakeVectorStore('./data_DL'))
...
with searh_tab:
    ...
    st.subheader('Setup')
    llm_name = st.selectbox('Which LLM?', ["Falcon-7B","OpenAI","Falcon-40B"])
